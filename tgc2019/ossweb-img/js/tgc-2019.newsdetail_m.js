...
module.exports = "<div class=\"header\">\r\n    <h1 class=\"title\">{sTitle}</h1>\r\n    <div class=\"info-bar\">\r\n        <p class=\"author\">作者：{sAuthor}</p>\r\n        <p class=\"time\">{sIdxTime}</p>\r\n    </div>\r\n</div>\r\n<div class=\"content\">\r\n    {sContent}\r\n</div>"
...
var _article = __webpack_require__("+F4Z");
...
var _index5 = __webpack_require__("TBuF");
...
function fillNewsData() {
    fillNews.detail({
        ...
        wrap: ".row-news-detail", // 模板处理完填充进来的容器
        tpl: _article2.default, // 模板字符串
        source: "web_m", //web_m | web_pc 数据上报用的，表明本页面用于pc或者移动
        // relWrap: ".rel", //相关推荐的容器
        // relTpl:
        //     '<p><a href="{url}"><img src="{sIMG}">{sTitle} {sCreated}</a></p>', // 相关推荐的dom结构模板
        callback: function callback(res) {
            var ret = (0, _utils.handleResponse)(res, "detail");
            if (ret.failed) {
                throw new Error(ret.msg);
            } else {
                var title = ret.sTitle;
                initShare(title);
            }
...
function initShare(title) {
    return new Promise(function (resolve) {
        $.getScript("//ossweb-img.qq.com/images/js/TGMobileShare/TGMobileShare.min.js", function () {
            TGMobileShare({
                shareTitle: title, //不设置或设置为空时，页面有title，则调取title
                shareDesc: "汇聚腾讯旗下头部IP，空降全世界最大单体建筑，线下沉浸式实景邀您体验。", //不设置或设置为空时，页面有Description，则调取Description
                shareImgUrl: __webpack_require__("ZmHP"), //分享图片尺寸200*200，且填写绝对路径
                shareLink: location.href, //分享链接要跟当前页面同域名(手Q分享有这个要求) ,不设置或设置为空时，默认调取当前URL
                actName: "index" //点击流命名，用于统计分享量，专题一般采用目录名称如a20151029demo
            });
            resolve();
        });
    });
...
function bindEvent() {
    $(document).on("tap", ".btn-back", function (ev) {
        var len = history.length;
        // 没有后退的页面，返回首页
        if (len < 2) {
            location.href = "https://tgc.qq.com/m/index.html";
        } else {
            history.back();
        }
    });
